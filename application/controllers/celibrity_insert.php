<?php
class celibrity_insert extends CI_Controller {
...
{
    $this->load->model('celibrity_model');
    $get_result = $this->celibrity_model->check_email_availablity();
    
    if(!$get_result )
    echo '<span style="color:#f00">Email exist. </span>';
    else
    echo '<span style="color:#063">Email OK</span>';
}

function check_mobile_availablity()
{
    $this->load->model('celibrity_model');
    $get_result = $this->celibrity_model->check_mobile_availablity();
    
    if(!$get_result )
    echo '<span style="color:#f00">Mobile exist. </span>';
    else
    echo '<span style="color:#063">Mobile OK</span>';
}
...
// Transfering Data To Model
        ...
        $this->email->to($email);
		$this->email->subject('Soulmate account confirmation mail');
		$name=$data['name'];
